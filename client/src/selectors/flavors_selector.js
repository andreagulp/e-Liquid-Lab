...
export const flavorUsageMetrics = createSelector(
  ...
  (flavorId, recipesByFlavor) => {
    ...
    const mlArray = recipesByFlavor.map(recipe => {
      return recipe.recipeFlavors
        .filter(flavor => flavor.flavorId === flavorId)
        .reduce((a, b) => a + b.ml, 0);
    });
...
export const getFlavorAlertList = createSelector([getFlavors], flavors =>
  flavors.inventoryFlavors.filter(flavor => flavor.alertList === true)
);
